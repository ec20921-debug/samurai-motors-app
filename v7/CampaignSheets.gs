...
function campaignOnOpen_() {
  ...
  ui.createMenu('📢 キャンペーン')
    ...
    .addSeparator()
    .addSubMenu(ui.createMenu('🗓 予約投稿（曜日・時刻で自動配信）')
      .addItem('予約シートを開く', 'openCampaignScheduleSheet')
      .addSeparator()
      .addItem('👁 選択した予約をプレビュー', 'previewScheduledRow')
      .addItem('🧪 選択した予約をテスト送信（自分へ）', 'testSendScheduledRow'))
    .addSeparator()
