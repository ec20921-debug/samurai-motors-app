...
### 📖 クメール語を書く前の必須ルール（仕組み化・スキップ禁止）

**キャンペーン配信・チラシ・SNS発信・予約Botの文言など、外部（顧客／現地スタッフ）へ
クメール語を出力するときは、書き始める前に必ず以下を読むこと。**

➡️ **`docs/marketing/khmer-phrasing-reference.md`**（ネイティブ素材から抽出した自然な言い回し集）

- 抽出パターン（`លោកអ្នក`＝丁寧な二人称／`ផ្តល់ជូន`＝敬語の提供／`សូម…`＝丁寧依頼／
  `តើ…មែនទេ?`＝丁寧な疑問 等）と語彙メモに沿って書く。
- 「クメール語がおかしい」と言われてきた問題の再発防止策。**思い出しベースで書かない**。
- 良い素材（他社広告・ネイティブの文）を見つけたら、このリファレンスにサンプルを追記して育てる。
- クメール語コピーは必ず**ネイティブ確認（Bopha/ロン）を促す注記**を添える（ドラフト扱い）。
